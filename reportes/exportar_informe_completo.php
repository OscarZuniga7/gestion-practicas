...
$pendientes = isset($_GET['pendientes']) ? (int)$_GET['pendientes'] : 0;
...
if ($pendientes) {
  $sql .= " AND (estado_informe='Pendiente' OR estado_evaluacion='No evaluado' OR estado_entrevista='Sin entrevista')";
}

$sql .= " ORDER BY estudiante, hito";
...
if ($formato === 'csv') {
  ...
  header('Content-Disposition: attachment; filename=informe_completo.csv');
...
if ($formato === 'xls') {
  ...
  header("Content-Disposition: attachment; filename=informe_completo.xls");
...
<head>
<meta charset="utf-8">
<title>Informe Completo (hecho y pendientes)</title>
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css">
</head>
<body class="p-4">
<div class="container">
  <h1 class="mb-3">Informe Completo por Hito (hecho + pendientes)</h1>
  ...
  <form class="row g-3 mb-4" method="get">
    ...
    <div class="col-md-3 d-flex align-items-end">
      <div class="form-check">
        <input class="form-check-input" type="checkbox" name="pendientes" value="1" id="chkPend" <?= $pendientes ? 'checked':''; ?>>
        <label class="form-check-label" for="chkPend">Solo pendientes</label>
      </div>
    </div>

    <div class="col-12 d-flex gap-2">
      <button class="btn btn-primary" type="submit">Filtrar</button>
      <a class="btn btn-success" href="?ini=<?=$ini?>&fin=<?=$fin?>&practica=<?=urlencode($practica)?>&pendientes=<?=$pendientes?>&formato=xls">Descargar XLS</a>
      <a class="btn btn-outline-secondary" href="?ini=<?=$ini?>&fin=<?=$fin?>&practica=<?=urlencode($practica)?>&pendientes=<?=$pendientes?>&formato=csv">CSV</a>
    </div>
  </form>
